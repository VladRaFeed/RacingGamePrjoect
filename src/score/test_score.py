...
from score import Scoreboard, get_scores
...
def scoreboard():
    ...
    with patch("score.MongoClient") as mock_mongo, \
        ...
        mock_mongo.return_value.racingDb.score.find.return_value.sort.return_value.limit.return_value = mock_get_scores.return_value
        mock_image = Mock()
...
def test_draw_renders_correctly(mock_draw_line, mock_surface, mock_small_font, mock_font, mock_set_mode, scoreboard):
...
@patch("score.small_font")
@patch("pygame.Surface")
def test_draw_empty_scores(mock_surface, mock_small_font, mock_font, mock_set_mode, scoreboard):
